...
public class LayerDragonhide extends LayerTiledOverlay<EntityLivingBase> {
	...
	public static final int durationBetweenChanges = 5;

	public LayerDragonhide(RenderLivingBase<?> renderer){
		super(renderer);
	}
	...
	//Chooses an incremental texture from the array
	@Override
	public ResourceLocation getTexture(EntityLivingBase entity, float partialTicks){
		return TEXTURES[(entity.ticksExisted / durationBetweenChanges) % ((TEXTURES.length - 1))];
	}
	...
	public void doRenderLayer(EntityLivingBase entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale){
		...
		GlStateManager.color(1, 1, 1, 1 - computeBlendFactor(entity, partialTicks));
	...
	private float computeBlendFactor(EntityLivingBase entity, float partialTicks) {
		float ticks = entity.ticksExisted % durationBetweenChanges;
		return Math.min(1.0f, partialTicks + ticks / durationBetweenChanges);
	}
