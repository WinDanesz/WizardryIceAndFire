...
public class ContainerDwarvenPocketForge extends Container {

	private final IInventory itemInventory;

	public ContainerDwarvenPocketForge(IInventory playerInventory, IInventory itemInventory, EntityPlayer player) {
		this.itemInventory = itemInventory;
		itemInventory.openInventory(player);
		if (itemInventory instanceof InventoryDwarvenPocketForge) {
			this.addSlotToContainer(new Slot(itemInventory, 0, 80, 36) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return ItemCharmDwarvenPocketForge.isItemValid(stack.getItem());
				}
				@Override
				public int getSlotStackLimit() {
					return ItemCharmDwarvenPocketForge.MAX_STACK_SIZE;
				}
			});
			// player's inventory
