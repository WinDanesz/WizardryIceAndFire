...
public class SummonMyrmexSentinel extends SpellMinionIFSP<EntityMyrmexSentinelMinion> {

	public static final String ARMOR_TOUGHNESS_MODIFIER = "minion_armor_toughness";

	public SummonMyrmexSentinel() {
		super("summon_myrmex_sentinel", EntityMyrmexSentinelMinion::new);
	}
	...
	protected boolean spawnMinions(World world, EntityLivingBase caster, SpellModifiers modifiers) {
		...
		if(!world.isRemote){
			for(int i=0; i<getProperty(MINION_COUNT).intValue(); i++){
				...
				EntityMyrmexSentinelMinion minion = createMinion(world, caster, modifiers);
