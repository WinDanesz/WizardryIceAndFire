...
public class HydraPulse extends SpellBuff {
	...
	protected boolean applyEffects(EntityLivingBase caster, SpellModifiers modifiers) {
		...
		if(healthPercentage < 1.0D /*&& caster.getHealth() > 0 maybe leave this out in case there's a funny interaction?*/){
			int level = this.getProperty(getStrengthKey(POTION)).intValue() + getBonusAmplifier(modifiers.get(SpellModifiers.POTENCY));
			...
			level *= this.getProperty(EFFECT_SCALE).floatValue();
