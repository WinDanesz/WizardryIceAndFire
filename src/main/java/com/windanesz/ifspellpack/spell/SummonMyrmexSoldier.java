...
import com.windanesz.ifspellpack.entity.living.EntityMyrmexSoldierMinion;
...
public class SummonMyrmexSoldier extends SpellMinionIFSP<EntityMyrmexSoldierMinion> {

	public static final String ARMOR_TOUGHNESS_MODIFIER = "minion_armor_toughness";

	public SummonMyrmexSoldier() {
		super("summon_myrmex_soldier", EntityMyrmexSoldierMinion::new);
	}
	...
	protected boolean spawnMinions(World world, EntityLivingBase caster, SpellModifiers modifiers) {
		...
		if(!world.isRemote){
			for(int i=0; i<getProperty(MINION_COUNT).intValue(); i++){
				...
				EntityMyrmexSoldierMinion minion = createMinion(world, caster, modifiers);
