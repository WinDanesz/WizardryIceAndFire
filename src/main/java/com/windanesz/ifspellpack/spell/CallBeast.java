	...
	public boolean cast(World world, EntityPlayer caster, EnumHand hand, int ticksInUse, SpellModifiers modifiers) {
		...
		if (data != null) {
			...
			if (uuids != null) {
				...
				RayTraceResult rayTrace = RayTracer.rayTrace(world, origin, endpoint, 0, false, true, false, Entity.class, null);
				if (rayTrace != null && rayTrace.typeOfHit == RayTraceResult.Type.ENTITY) {
					...
					if (isAcceptableBeast(target)) {
						if (!containsMount(uuids, world, target)) {
							uuids.add(target.getUniqueID());
						} else {
							replaceMount(uuids, world, target);
						}
						return false;
					}
				} else {
