...
public class CockatricesStare extends Spell {
	...
	public CockatricesStare() {
		...
		this.addProperties(VIEW_RADIUS);
	}
	...
	public boolean cast(World world, EntityPlayer caster, EnumHand hand, int ticksInUse, SpellModifiers modifiers) {
		if (!EntityGorgon.isBlindfolded(caster)) {
			...
			if (ItemArtefact.isArtefactActive(caster, WizardryItems.amulet_anchoring)) {
			...
			for (EntityLivingBase target : targets) {
				int attackStrength = baseDamage + this.getFriendsCount(allies, target);
	...
	public boolean cast(World world, EntityLiving caster, EnumHand hand, int ticksInUse, EntityLivingBase target, SpellModifiers modifiers) {
		if (!EntityGorgon.isBlindfolded(caster)) {
			...
			for (EntityLivingBase target1 : targets) {
				target1.addPotionEffect(new PotionEffect(MobEffects.WITHER, 10, 2 + Math.min(1, baseDamage)));
				target1.addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, 10, Math.min(4, baseDamage)));
				target1.addPotionEffect(new PotionEffect(MobEffects.NAUSEA, 200, 0));
				if (baseDamage >= 2 && target1.ticksExisted % 40 == 0) {
					target1.attackEntityFrom(DamageSource.WITHER, baseDamage - 1);
				}
				double d5 = this.getAttackAnimationScale(ticksInUse);
	...
	public int getFriendsCount(List<EntityPlayer> allies, EntityLivingBase target) {
		...
		for (EntityPlayer ally : allies) {
			if (EntityGorgon.isEntityLookingAt(ally, target, view_radius) && EntityGorgon.isEntityLookingAt(target, ally, view_radius)) {
				i++;
			}
		}
		return i;
