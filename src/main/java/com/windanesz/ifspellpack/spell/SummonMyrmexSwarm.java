...
import com.windanesz.ifspellpack.entity.living.EntityMyrmexSwarmerMinion;
...
public class SummonMyrmexSwarm extends SpellMinionIFSP<EntityMyrmexSwarmerMinion> {

	public static final String ARMOR_TOUGHNESS_MODIFIER = "minion_armor_toughness";

	public SummonMyrmexSwarm() {
		super("summon_myrmex_swarm", EntityMyrmexSwarmerMinion::new);
		this.flying = true;
	}
	...
	protected boolean spawnMinions(World world, EntityLivingBase caster, SpellModifiers modifiers) {
		...
		if(!world.isRemote){
			for(int i=0; i<getProperty(MINION_COUNT).intValue(); i++){
				...
				EntityMyrmexSwarmerMinion minion = createMinion(world, caster, modifiers);
