...
import com.windanesz.ifspellpack.client.gui.GuiCallDragon;
...
public class S2CPacketCallDragon implements IMessageHandler<S2CPacketCallDragon.Message, IMessage> {

	@Override
	public IMessage onMessage(Message message, MessageContext ctx){
		if(ctx.side.isClient()){
			Minecraft.getMinecraft().addScheduledTask(() -> {
				Minecraft.getMinecraft().displayGuiScreen(new GuiCallDragon(message.enabledMounts));
			});
		}
		return null;
	}
